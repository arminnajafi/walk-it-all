...
The map supports native rotation and the standard compass. The first location-button tap enters heading follow so MapKit can present the device heading on its standard user-location indicator; the next tap switches to north-up follow, and later taps alternate between them. Starting or resuming Live Trail also requests heading follow. Normal map gestures return the camera to free mode through MapKit's tracking delegate.
