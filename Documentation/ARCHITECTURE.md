...
The map supports native rotation and the standard compass. A viewport command cycles from free camera to north-up follow, then heading follow, then back to north-up follow. Starting or resuming Live Trail requests heading follow; normal map gestures return the camera to free mode through MapKit's tracking delegate.
