...
The map supports native rotation and the standard compass. The location button follows the user's position without following heading, so the camera never turns as the phone turns. A heading-only Core Location manager lives inside the narrow MapKit coordinator and rotates a translucent direction fan around the standard blue position dot. It does not request permission, run location delivery, or publish compass updates through SwiftUI. Starting or resuming Live Trail uses the same position-follow behavior. Normal map gestures return the camera to free mode through MapKit's tracking delegate; the fan remains aligned with the real world even when the user rotates the map manually.
